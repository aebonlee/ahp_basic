...
| `84bbd25` | fix: 기준 종합중요도 바 굵기 통일 + 라벨 한줄 표시 |
...
## 5. 기준 종합중요도 바 굵기 통일 + 라벨 한줄 표시

### 배경
- 상위기준 `.barTrack`에 `padding-top: 10px` + `border-top: 1px` → `box-sizing: border-box`에서 바 내용 영역 9px로 축소 (하위기준은 20px) → 바 굵기 불일치
- 인덴트 `level * 16px`이 라벨을 넓혀 하위기준 이름이 2줄로 표시

### 변경 내용

| 파일 | 변경 |
|------|------|
| `src/components/results/ResultSummary.jsx` | 구분선을 `barDivider` 별도 요소(`grid-column: 1/-1`)로 분리, 인덴트 축소(16→12px) |
| `src/styles/results.module.css` | `padding-top`/`border-top` 셀 해킹 전부 제거, `.barDivider` 추가, 모든 `.barTrack` height 20px 통일 |

### 결과
- 모든 행의 바 높이 20px 통일
- 구분선은 그리드 전체 너비를 차지하는 별도 요소로 깔끔하게 분리
- 라벨 한줄 표시 유지
...
| 6 | `src/components/results/ResultSummary.jsx` | 상위/하위 기준 시각 구분 + 정렬 + 바 굵기 통일 |
| 7 | `src/styles/results.module.css` | CSS Grid 정렬 + 상위/하위 스타일 + barDivider |

**총 7개 파일 수정, 커밋 4건, 문서 3개 추가**
