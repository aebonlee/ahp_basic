...
export default function SensitivityPage() {
  ...
  const analysisData = useMemo(() => {
    ...
    const altPrioritiesByCriterion = rootCriteria.map(rootC => {
      // For simplicity, use leaf criteria under this root
    ...
    const data = sensitivityAnalysis(rootResult.priorities, altPrioritiesByCriterion, selectedCriterion);
  ...
  return (
    <PageLayout>
      ...
      {analysisData ? (
        ...
        <p style={{ color: 'var(--color-text-muted)' }}>분석할 데이터가 없습니다.</p>
      )}
    </PageLayout>
  );
}
